...
public class ReportGenerator {
    ...
    public void inspectStepDefinitions() {
        /*
         * Reflections reflections = new
         * Reflections("com.nature.quickstep.stepdefs.*"); Set<Method>
         * allMethods = reflections.getMethodsAnnotatedWith(When.class);
         * 
         * System.out.println("====>*****<====" ); for (Method m : allMethods) {
    ...
    public void generateReportBodyForScenariosReport() {
        ...
        report("<html>");
        report("<head><title>Scenarios Lookup</title></head>");
        report("<body onload=\"populateScenariosTextArea()\">");
        report("<H1>Scenarios Lookup</H1>");
        report("<table>");
    ...
    public void generateJavascriptFunctionsForStepsReport() {
        ...
        while (iterator.hasNext()) {
            ...
            report("allSteps[" + i + "] = \"" + curStep.replace("\"", "\\\"") + "\";");
            i++;
        }
    ...
    public void generateReportBodyForStepsReport() {
        report("<html>");
        report("<head><title>DSL Lookup</title></head>");
        report("<body onload='populateStepsList()'>");
        report("<H1>DSL Lookup</H1>");
    ...
    public void parseCucumberJSON() {
        ...
        features = null;
        Gson gson = new GsonBuilder().registerTypeAdapter(Step.class, new InstanceCreator<Step>() {
            @Override
            public Step createInstance(Type type) {
                return new Step(new ArrayList<Comment>(), "", "", new Integer(0), new ArrayList<DataTableRow>(),
                        new DocString("", "", new Integer(0)));
            }
        }).registerTypeAdapter(Tag.class, new InstanceCreator<Tag>() {
            @Override
            public Tag createInstance(Type type) {
                return new Tag("", new Integer(0));
            }
        }).create();
